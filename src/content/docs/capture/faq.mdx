...
Go to the client settings and click **Disconnect from Xero**. This stops all syncing and prevents new documents from being published. Documents already in Capture are kept, and anything already published to your accounting platform remains unchanged.
...
Yes. Open the client settings and click **Reconnect to Xero**. Reconnecting triggers a fresh data sync, and you can resume processing and publishing documents.

### A client shows "Disconnected" or "Connection failed". What happened?

Capture's access to that Xero organisation has been removed, usually because someone revoked it from inside Xero. Otto checks each client against Xero once a day and updates the status, so a Xero-side disconnect shows as **Disconnected**. If a sync or publish reaches the revoked connection first, you'll see **Connection failed** instead. Either way, organisation admins get a notification. To fix it, open the client settings and click **Reconnect to Xero**, or use the **Reconnect** action in the **Needs setup** list on the **Clients** page.
...
Documents that are discarded after extraction count as processed, since Otto's extraction work has already been completed.
