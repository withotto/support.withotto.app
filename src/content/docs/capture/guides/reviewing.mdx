...
<ZoomableImage
  src={reviewInterface}
  alt="The review screen, with the original document on one side and the extracted fields on the other: supplier, dates, amounts, and the line items with their nominal codes and tax rates. Above them, arrows and the position in the queue, reading 5 of 6"
  label="Review screen"
/>
...
If the sender added a note, such as a WhatsApp caption or a note written on the [client missing receipts page](/capture/missing-receipts/client-page), it appears under the filename as **Note from sender**. Otto reads the note when he extracts the document. A note from the client missing receipts page can change afterwards, and if it does, the review screen tells you Otto read the document before the note changed, and shows what it said before.
...
- Tracking categories on Xero, or classes on QuickBooks Online
...
- **The organisation can use multiple currencies, and the platform allows currencies to be added.** The warning offers to add the currency, or to convert the amounts to the base currency instead. This is the case on Xero where the plan includes multi-currency, and on QuickBooks Online where multi-currency is turned on (see [Xero currencies](/capture/platforms/xero#currencies) and [QuickBooks Online currencies](/capture/platforms/quickbooks#currencies))
...
- Currency not available in your accounting platform (not enabled on Xero or QuickBooks Online, or not supported by FreeAgent)
...
3. **An attachment:** The original document file is attached to bills on every platform. On FreeAgent, sales invoices have no attachment field, so only Xero invoices keep the source file (see [Xero](/capture/platforms/xero#attachments), [FreeAgent](/capture/platforms/freeagent#attachments), and [QuickBooks Online](/capture/platforms/quickbooks#attachments))

On Xero, the bill or invoice is created in the state you selected. FreeAgent and QuickBooks Online have no settable state (see publishing to [FreeAgent](/capture/platforms/freeagent#publishing) and [QuickBooks Online](/capture/platforms/quickbooks#publishing)).
